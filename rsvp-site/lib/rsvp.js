...
const FAMILY_SIDES = {
  akhila: {
    ...
    person: 'Akhila',
    family: 'Chennaboina',
    pelliCeremonyName: 'Pellikuthuru',
    pelliSubtitle: 'Bride-side nalugu and blessings',
    pelliVenue: process.env.PELLIKUTHURU_ADDRESS || process.env.AKHILA_SIDE_ADDRESS || 'Pellikuthuru address to be confirmed'
  },
  akshay: {
    label: "Akshay's side",
    person: 'Akshay',
    family: 'Lenkalapally',
    pelliCeremonyName: 'Pellikoduku',
    pelliSubtitle: 'Groom-side nalugu and blessings',
    pelliVenue: process.env.PELLIKODUKU_ADDRESS || process.env.AKSHAY_SIDE_ADDRESS || 'Pellikoduku address to be confirmed'
  }
};
...
function getGuestSideDetails(guest) {
  const side = normalizeGuestSide(guest?.side);
  return side ? { id: side, ...FAMILY_SIDES[side] } : null;
}
...
function getInvitedEvents(guest) {
  ...
  return events.map(event => {
    ...
    return {
      ...
      subtitle: sideDetails.pelliSubtitle,
      venue: sideDetails.pelliVenue,
      side: sideDetails.label
    };
  });
}
