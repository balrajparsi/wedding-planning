...
const COMMON_USES_BRIDE_SIDE_ADDRESS = isPlaceholderAddress(RAW_COMMON_EVENT_ADDRESS);
...
function resolveBrideSideAddress(value) {
  ...
  return address && !isPlaceholderAddress(address) ? address : DEFAULT_BRIDE_SIDE_ADDRESS;
}

function resolveOptionalAddress(value) {
  const address = String(value || '').trim();
  return address && !isPlaceholderAddress(address) ? address : 'Location to be confirmed';
}
