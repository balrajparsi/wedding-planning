...
const COMMON_USES_BRIDE_SIDE_ADDRESS = isBrideSideAddressPlaceholder(RAW_COMMON_EVENT_ADDRESS);
const COMMON_EVENT_ADDRESS = COMMON_USES_BRIDE_SIDE_ADDRESS
  ? PELLIKUTHURU_ADDRESS
  : RAW_COMMON_EVENT_ADDRESS || PELLIKUTHURU_ADDRESS;
const COMMON_EVENT_MAP_URL = process.env.COMMON_EVENT_MAP_URL || process.env.WEDDING_COMMON_MAP_URL || (
  COMMON_USES_BRIDE_SIDE_ADDRESS || !RAW_COMMON_EVENT_ADDRESS ? PELLIKUTHURU_MAP_URL : ''
);
const RAW_SATYANARAYANA_ADDRESS = process.env.SATYANARAYANA_ADDRESS || process.env.SATYANARAYANA_SWAMY_VRATAM_ADDRESS || RAW_COMMON_EVENT_ADDRESS;
const SATYANARAYANA_ADDRESS = resolveBrideSideAddress(RAW_SATYANARAYANA_ADDRESS);
const SATYANARAYANA_MAP_URL = resolveMapUrl(
  process.env.SATYANARAYANA_MAP_URL || process.env.SATYANARAYANA_SWAMY_VRATAM_MAP_URL || COMMON_EVENT_MAP_URL,
  DEFAULT_BRIDE_SIDE_MAP_URL
);

function isBrideSideAddressPlaceholder(value) {
  const normalized = String(value || '').trim().toLowerCase().replace(/[-_]+/g, ' ').replace(/\s+/g, ' ');
  return normalized === 'bride side address';
}
...
function resolveBrideSideAddress(value) {
  const address = String(value || '').trim();
  return address && !isBrideSideAddressPlaceholder(address) ? address : DEFAULT_BRIDE_SIDE_ADDRESS;
}

function resolveOptionalAddress(value) {
  const address = String(value || '').trim();
  return address && !isBrideSideAddressPlaceholder(address) ? address : 'Location to be confirmed';
}

function resolvePellikodukuAddress(value) {
  const address = resolveOptionalAddress(value);
  const normalized = normalizeAddress(address);
  return normalized && normalized !== normalizeAddress(PELLIKUTHURU_ADDRESS) && normalized !== normalizeAddress(DEFAULT_BRIDE_SIDE_ADDRESS)
    ? address
    : 'Location to be confirmed';
}

function resolveMapUrl(value, fallback) {
...
const RSVP_EVENTS = [
  ...
  },
  {
    ...
    locations: [
      ...
      {
        label: 'Pellikoduku',
        venue: PELLIKODUKU_ADDRESS,
        mapUrl: PELLIKODUKU_MAP_URL
      }
    ]
  },
  {
...
function eventVenueText(event) {
  ...
  if (!locations.length) return event.venue;
  return locations
...
function buildCalendarFile(guest) {
  ...
  events.forEach(event => {
    ...
    const venueText = eventVenueText(event);
    lines.push(`LOCATION:${escapeIcsText(venueText)}`);
    lines.push(`DESCRIPTION:${escapeIcsText(`${event.displayDate} · ${event.time}\n${event.subtitle}\nVenue: ${venueText}${eventMapText(event)}`)}`);
