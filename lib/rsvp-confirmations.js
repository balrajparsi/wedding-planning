...
function sanitizeMimeFilename(value) {
  return String(value || 'attachment')
    .replace(/[\r\n"\\]/g, '_')
    .trim() || 'attachment';
}

function buildGmailRawMessage({ from, to, subject, html, attachments = [] }) {
  ...
  if (!attachments.length) {
    ...
    const boundary = `----=_AkhilaAkshay_${Date.now()}_${Math.random().toString(16).slice(2)}`;
    const parts = [
      `--${boundary}`,
      'Content-Type: text/html; charset="UTF-8"',
      'Content-Transfer-Encoding: base64',
