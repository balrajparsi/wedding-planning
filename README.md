...
- **Guest Side**: Akhila/Akshay side changes Pellikuthuru/Pellikoduku details and addresses. The combined Pellikuthuru / Pellikoduku confirmation card shows separate location subheadings for `PELLIKUTHURU_ADDRESS` and `PELLIKODUKU_ADDRESS`. Pellikuthuru falls back to `510 Peach Ave, Centerton, AR 72719`; Pellikoduku falls back to `1404 Robinhood Road, Bentonville, AR 72712`. Haldi uses `COMMON_EVENT_ADDRESS` when set, otherwise it defaults to the Pellikuthuru / bride-side address. Satyanarayana Swamy Vratam uses `SATYANARAYANA_ADDRESS`. Bulk reminder emails link to the YouTube video invitation instead of attaching the video file.
- **Venue Seeds**: When the Venues KV list is empty, the Venues API seeds the known RSVP venues from `RSVP_EVENTS`: Haldi, Sangeeth, Pellikuthuru / Pellikoduku, Marriage, and Satyanarayana Swamy Vratam.
